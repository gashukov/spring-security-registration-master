...
public class LocalizationIntegrationTest {
    ...
    @Test
    public void given_theLanuageParamterIsRussian_then_the_title_of_the_log_page_is_Voyti() {
        final RequestSpecification request = RestAssured.given().param("lang", "ru_RU");
        request.when().get("/login").then().assertThat().statusCode(200).and().body(containsString("<h1>Войти</h1>"));
    }
}
